...
PROVISION_GAP_BY_METRIC_REGION_DIR = STORY_DIR / "provision_gap_by_metric_region"
...
BROWSER_COLUMNS = [
    # Keep browser records deliberately slim. Source details live in manifests, not repeated on every row.
    "source_domain",
    "time_period",
    "time_identifier",
    "academic_year",
...
BENCHMARK_INPUT_COLUMNS = sorted(set(
    CORE_DIMENSIONS
    + LA_COLUMNS
    + SOURCE_COLUMNS
    + ["old_la_code"]
    + [spec.column for spec in METRICS]
    + [spec.numerator for spec in METRICS if spec.numerator]
    + [spec.denominator for spec in METRICS if spec.denominator]
))
...
def load_region_records(manifest: dict[str, Any]) -> pd.DataFrame:
    ...
    for region in manifest.get("regions", []):
        ...
        keep_cols = [col for col in BENCHMARK_INPUT_COLUMNS if col in frame.columns]
...
def write_provision_gap_story_files(la_metrics: pd.DataFrame, pretty: bool = False) -> dict[str, Any]:
    """
    Write browser-facing provision-gap story files, split metric and region

    page fetchs one region-sized file for selected metric and region
    (file sizes otherwise too huge)
    """
    out_dir = PROVISION_GAP_BY_METRIC_REGION_DIR
    ...
    for metric, metric_part in df.groupby("metric", dropna=False, sort=True):
        ...
        for region_code, part in metric_part.groupby("region_code", dropna=False, sort=True):
            ...
            payload = {
                ...
                "record_count": int(len(part)),
                "provision_labels": sorted(str(v) for v in part["send_category"].dropna().unique()),
                "records": records_to_jsonable(part),
            }
            ...
            region_files[region_code] = {
                "region_code": region_code,
                "region_label": region_label,
                "file": f"data/benchmarks/story/provision_gap_by_metric_region/{safe_file_part(metric)}/{filename}",
                "record_count": int(len(part)),
            }
...
def add_derived_reason_rates(df: pd.DataFrame) -> pd.DataFrame:
    """
    Add derived attendance reason rates per 1000 possible sessions

    for benchmarking than raw session counts as adjust for size 
    of the pupil/session denominator
    """
    out = df
...
def compute_group_metric(group: pd.DataFrame, spec: MetricSpec) -> tuple[float | None, float | None, float | None]:
    ...
    else simple mean for rates/% or sum for count metrics
    """
...
def build_metric_rows(
    ...
    """
    Build metric rows using metric-by-metric vectorised groupby ops

    only groups rows that actually contain the metric being built
    """
    ...
    for index, spec in enumerate(metric_specs, start=1):
        ...
        # Numerator + denominator metrics are recalc from summed components
        # used for % and derived per-1000 session rates
        ...
        # Denominator-only metrics use weighted mean
        ...
        # Remaining metrics use simple mean
        else:
            part = part[part["_value"].notna()]
...
def reduce_for_json(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Keep -time periods- as strings so browser doesnt display 202,526 from dfe yr
...
def build_benchmarks(
    ...
    benchmark_manifest = {
        ...
        "outputs": {
            ...
            "story": "data/benchmarks/story/",
            "provision_gap_by_metric_region": "data/benchmarks/story/provision_gap_by_metric_region/",
            "primary_need_by_metric_region": "data/benchmarks/story/primary_need_by_metric_region/",
        },
        "story_outputs": {
            "provision_gap_by_metric_region": "data/benchmarks/story/provision_gap_by_metric_region/",
            "primary_need_by_metric_region": "data/benchmarks/story/primary_need_by_metric_region/",
        ...
        "provision_gap_files": provision_gap_files,
        "primary_need_files": primary_need_files,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Write full docs/data/benchmarks/la_by_metric/*.json files. These large debug outputs not needed for browser/benchmarking story pages",
    )
